...
One historically important padding scheme is \textbf{PKCS\#1 v1.5}. Let $k$ be the byte length of the modulus $N$. Before RSA encryption, a message $M$ of $\mathrm{mLen}$ bytes is embedded into a $k$-byte encoded block of the form
\[
...
\item produces a structured RSA input in which every output bit depends on the entire message and seed.
\end{itemize}
...
\end{tikzpicture}
\caption{OAEP combines the data block DB and a random seed through two mask generation functions $G$ and $H$ before RSA is applied. Each output bit of $\mathrm{EM}$ depends on the whole message and the whole seed.}
\end{figure}
